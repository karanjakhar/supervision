...
class DwellHeatMapAnnotator:
    """
    A class for drawing dwell (no or very small movement) heatmaps on an image based on provided detections.
    Heat accumulates over time and is drawn as a semi-transparent overlay
    of blurred circles.
    """
    ...
    def annotate(self, scene: np.ndarray, detections: Detections) -> np.ndarray:
        ...
        if self.heat_mask is None:
            self.heat_mask = np.zeros(scene.shape[:2])
        mask = np.zeros(scene.shape[:2])
        for xy, tracker_id in zip(detections.get_anchors_coordinates(self.position), detections.tracker_id):
            movement = self.trace.get_movement(tracker_id)
